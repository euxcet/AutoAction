...
def export_pt(model, PT_PATH, sequence_dim, channel_dim, use_cuda):
    if use_cuda:
        libtorch_model = torch.jit.trace(model, torch.rand(1, sequence_dim, channel_dim).cuda())
    else:
        libtorch_model = torch.jit.trace(model, torch.rand(1, sequence_dim, channel_dim))
